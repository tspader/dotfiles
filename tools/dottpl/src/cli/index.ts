...
export function help(
  ...
  if (pos && Object.keys(pos).length > 0) {
    ...
    for (const [k, v] of Object.entries(pos)) {
      ...
      if (v.default !== undefined) desc += ` ${t.dim(`(default: ${String(v.default)})`)}`;
  ...
  if (Object.keys(opts).length > 0) {
    ...
    for (const [k, v] of Object.entries(opts)) {
      ...
      if (v.default !== undefined && v.type !== "boolean") {
        desc += ` ${t.dim(`(default: ${String(v.default)})`)}`;
      }
...
function configure(y: any, def: CommandDef | CliDef, root: string, path: string[]): void {
  if ("positionals" in def && def.positionals) {
    for (const [k, v] of Object.entries(def.positionals)) {
      y.positional(k, {
        type: v.type,
...
function command(y: any, name: string, def: CommandDef, root: string, path: string[]): void {
  let cmd = name;
  if (def.positionals) {
    for (const [k, v] of Object.entries(def.positionals)) {
      cmd += v.required ? ` <${k}>` : ` [${k}]`;

...
typedef enum {
  SPN_JOB_KIND_SYNC,
  SPN_JOB_KIND_CONFIGURE,
  SPN_JOB_KIND_BUILD,
  SPN_JOB_KIND_TEST,
  SPN_JOB_KIND_CLEANUP,
  SPN_JOB_KIND_COUNT,
} spn_job_kind_t;

sp_typedef_fn(spn_job_result_t, spn_job_init_fn_t, sp_app_t*);
sp_typedef_fn(spn_job_result_t, spn_job_update_fn_t, sp_app_t*);
sp_typedef_fn(spn_job_kind_t, spn_job_exit_fn_t, sp_app_t*);

typedef struct {
  spn_job_kind_t kind;
  struct {
    spn_job_init_fn_t init;
    spn_job_update_fn_t update;
    spn_job_exit_fn_t exit;
  } vtable;
} spn_job_t;

spn_job_result_t spn_job_build_init(sp_app_t* sp);
spn_job_result_t spn_job_build_update(sp_app_t* sp);
spn_job_kind_t   spn_job_build_exit(sp_app_t* sp);
spn_job_result_t spn_job_sync_init(sp_app_t* sp);
spn_job_result_t spn_job_sync_update(sp_app_t* sp);
spn_job_kind_t   spn_job_sync_exit(sp_app_t* sp);
spn_job_result_t spn_job_configure_init(sp_app_t* sp);
spn_job_result_t spn_job_configure_update(sp_app_t* sp);
spn_job_kind_t   spn_job_configure_exit(sp_app_t* sp);
spn_job_result_t spn_job_test_init(sp_app_t* sp);
spn_job_result_t spn_job_test_update(sp_app_t* sp);
spn_job_kind_t   spn_job_test_exit(sp_app_t* sp);
spn_job_result_t spn_job_cleanup_init(sp_app_t* sp);
spn_job_result_t spn_job_cleanup_update(sp_app_t* sp);
spn_job_kind_t   spn_job_cleanup_exit(sp_app_t* sp);
...
typedef struct {
  ...
  spn_job_t jobs [SPN_JOB_KIND_COUNT];
...
void spn_app_prepare_build(spn_app_t* app) {
  ...
  sp_ht_for_kv(app->build.contexts.deps, it) {
    ...
    spn_bg_id_t sync = spn_bg_add_fn(&app->build.graphs.sync, spn_executor_sync_repo, dep);
    spn_bg_tag_command(&app->build.graphs.sync, sync, sp_format("sync ({})", SP_FMT_STR(dep->ctx.name)));
  ...
  sp_ht_for_kv(app->build.contexts.bins, it) {
    ...
    spn_bg_tag_command(&app->build.graph, nodes.build, sp_format("spn_executor_bin({})", SP_FMT_STR(ctx->name)));
...
spn_app_t spn_app_new() {
  spn_app_t app = {
    .jobs = {
      [SPN_JOB_KIND_SYNC] = {
        .kind = SPN_JOB_KIND_SYNC,
        .vtable = {
          .init = spn_job_sync_init,
          .update = spn_job_sync_update,
          .exit = spn_job_sync_exit,
        }
      },
      [SPN_JOB_KIND_BUILD] = {
        .kind = SPN_JOB_KIND_BUILD,
        .vtable = {
          .init = spn_job_build_init,
          .update = spn_job_build_update,
          .exit = spn_job_build_exit,
        }
      },
    }
  };
  ...
  if (sp_atomic_s32_get(&ex->shutdown)) {
    ...
    switch (error.some) {
      ...
      case SP_OPT_NONE: {
        ...
        spn_event_buffer_push_ex(spn.events, &build->contexts.package, (spn_build_event_t) {
          .kind = SPN_BUILD_EVENT_BUILD_PASSED,
          .done = {
            .time = build->executor->elapsed
          }
        });
...
spn_job_kind_t spn_job_build_exit(sp_app_t* sp) {
  ...
  if (app->config.tasks.tests) {
    return SPN_JOB_KIND_TEST;
  }

  return SPN_JOB_KIND_CLEANUP;
}
...
spn_cli_result_t spn_cli_graph(spn_cli_t* cli) {
  ...
  if (sp_str_valid(cli->graph.output)) {
    ...
    spn_bg_to_mermaid(&app.build.graph, &stream);
    sp_io_close(&stream);
  } else {
    spn_bg_to_mermaid(&app.build.graph, &spn.logger.out);
  }
  return SPN_CLI_DONE;
}
